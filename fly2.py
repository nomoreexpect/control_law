...
async def control(drone,PositionNedYaw, initial_position, exi):
    ...
    async for p in drone.telemetry.position_velocity_ned():
        ...
        combin_vector = np.vstack((ex,ey,ez))
        combin_state = np.vstack((x,y,z))
        exi = np.vstack((combin_state, combin_vector))
        exi = exi*[[0.2],[5],[0.4],[0.5],[5],[5]]
